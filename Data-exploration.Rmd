...
datasummary((`Distance to nearest road (km)`=rdist ) + (`Population within 8 miles`=sumpop) + (`Population within cell`=sumpop2) + (`Distance to nearest wetland (km)`=wtlddist) + (`Mean annual precipitation (mm/yr)`=meanprec) + (`Mean annual air temp (C)`=meanat) + (`Mean annual sea surface temp (C)`=meanSST) + (`Mean annual chlorophyll a (mg/m3)`=ChA) ~ Mean + Median + SD + Histogram, data = df[df$hres==250,],fmt="%.2f", title = 'Continuous Predictors 250m')
...
cormat<-df.nosf[df.nosf$hres==250,] %>%
  dplyr::select(sumpop,sumpop2,rdist,BIKE_PATH,BOATING,BT_FACIL_T,CAMPGROUND,DSABLDACSS,FEE,FISHING,PARKING,PTH_BEACH,RESTROOMS,STRS_BEACH,WaterTrail,BayTrail,wtlddist,meanprec,meanat,meanSST,ChA,RockyShore,Armored,Marshes,SandyBeach,TidalFlat)
