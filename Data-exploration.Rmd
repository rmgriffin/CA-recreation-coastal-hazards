...
# Packages
PKG <- c("tidyverse","knitr", "renv", "modelsummary", "kableExtra", "corrplot","RColorBrewer","lmtest","sandwich","pscl", "glmmTMB", "broom.mixed","vcd", "lmtest", "sf", "tmap", "tigris", "patchwork", "rnaturalearth", "rgeos", "ggrepel", "ggpubr", "margins", "spdep", "shiny","adespatial", "ade4", "adegraphics", "maptools","glmmfields", "R2BayesX", "BayesXsrc","CARBayes","coda","furrr","hglm","RANN") 
...
datasummary((`Distance to nearest road (km)`=rdist ) + (`Population within 8 miles`=sumpop) + (`Distance to nearest wetland (km)`=wtlddist) + (`Mean annual precipitation (mm/yr)`=meanprec) + (`Mean annual air temp (C)`=meanat) + (`Mean annual sea surface temp (C)`=meanSST) + (`Mean annual chlorophyll a (mg/m3)`=ChA) ~ Mean + Median + SD + Histogram, data = df[df$hres==250,],fmt="%.2f", title = 'Continuous Predictors 250m')
...
cormat<-df.nosf[df.nosf$hres==1000,] %>%
  dplyr::select(sumpop,rdist,BIKE_PATH,BOATING,BT_FACIL_T,CAMPGROUND,DSABLDACSS,FEE,FISHING,PARKING,PTH_BEACH,RESTROOMS,STRS_BEACH,WaterTrail,BayTrail,wtlddist,meanprec,meanat,meanSST,ChA,RockyShore,Armored,Marshes,SandyBeach)
...
Correlogram of other predictors. Relationships are stable across resolutions; the above plots correlations at 1000m resolution. Strong correlations (>|0.5|) are mainly observed between the atmospheric and water characteristics. Air temperature, precipitation, and water temperature in particular.
...
```{r `Poisson version, including cell`, echo=FALSE}
df$meanat2<-df$meanat^2

Pois.reg<-function(tipe,dv){ # tipe is either "PUD","TUD","DC" / rs is one of {25, 500, 1000} / dv is either "ud" or "ihsud" or "lnud"
  models <- list()
  models.rob<-list()
  models[['p250']]<-glm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = dv), data = df[df$source==tipe&df$hres==250,], family = poisson)
  models.rob[['p250']]<-coeftest(models[['p250']], vcov = sandwich)
  
  models[['p500']]<-glm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = dv), data = df[df$source==tipe&df$hres==500,], family = poisson)
  models.rob[['p500']]<-coeftest(models[['p500']], vcov = sandwich)
  
  models[['p1000']]<-glm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = dv), data = df[df$source==tipe&df$hres==1000,], family = poisson)
  models.rob[['p1000']]<-coeftest(models[['p1000']], vcov = sandwich)

  return(list(models,models.rob))
}

PUD.p<-Pois.reg("PUD","ud")
PUD.p[[2]]
TUD.p<-Pois.reg("TUD","ud")
TUD.p[[2]]
DC.p<-Pois.reg("DC","ud")
DC.p[[2]]
...
The cellular data has convergence issues with most of the regression formats, so we re-estimate models using the Poisson regression

```{r `OLS version, including cell`, echo=FALSE}
OLS.reg<-function(tipe,dv){ # tipe is either "PUD","TUD","DC" / rs is one of {25, 500, 1000} / dv is either "ud" or "ihsud" or "lnud"
  models <- list()
  models.rob<-list()
  models[['p250']]<-lm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = dv), data = df[df$source==tipe&df$hres==250,])
  models.rob[['p250']]<-coeftest(models[['p250']], vcov = sandwich)
  
  models[['p500']]<-lm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = dv), data = df[df$source==tipe&df$hres==500,])
  models.rob[['p500']]<-coeftest(models[['p500']], vcov = sandwich)
  
  models[['p1000']]<-lm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = dv), data = df[df$source==tipe&df$hres==1000,])
  models.rob[['p1000']]<-coeftest(models[['p1000']], vcov = sandwich)

  return(list(models,models.rob))
}

PUD.o<-OLS.reg("PUD","ud")
PUD.o[[2]]
TUD.o<-OLS.reg("TUD","ud")
TUD.o[[2]]
DC.o<-OLS.reg("DC","ud")
DC.o[[2]]
...
```{r `Poisson residuals`, echo=FALSE}
rm(PUD.p,PUD.o,TUD.p,TUD.o,DC.o,DC.p)
PUD.p<-glm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = "ud"), data = df[df$source=="PUD"&df$hres==250,], family = poisson)
TUD.p<-glm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = "ud"), data = df[df$source=="TUD"&df$hres==250,], family = poisson)
DC.p<-glm(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat","meanat2"),response = "ud"), data = df[df$source=="DC"&df$hres==250,], family = poisson)
...
MC.PUD.p<-moran.mc(df250PUD.p$PUDres,nb.list.PUD.p,nsim = 999,zero.policy = TRUE)
MC.PUD.p

df250TUD.p<-st_centroid(df250TUD)
nb5.TUD<-knn2nb(knearneigh(df250TUD.p, k=5))
nb.list.TUD.p<-nb2listw(nb5.TUD, style = "B", zero.policy = TRUE)
MC.TUD.p<-moran.mc(df250TUD.p$TUDres,nb.list.TUD.p,nsim = 999,zero.policy = TRUE)
MC.TUD.p

df250DC.p<-st_centroid(df250DC)
nb5.DC<-knn2nb(knearneigh(df250DC.p, k=5))
nb.list.DC.p<-nb2listw(nb5.DC, style = "B", zero.policy = TRUE)
...
fit_model<-function(x) S.glm(ud ~ RockyShore+SandyBeach+Marshes+Armored+sumpop+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+RESTROOMS+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2, data = x, family = "poisson", burnin=10000, n.sample=200000, thin=5)
...
nb.PUD<-poly2nb(df250PUD)
nb.list.PUD<-nb2listw(nb.PUD, style = "B")
MC.PUD<-moran.mc(df250PUD$pearson,nb.list.PUD,nsim = 999) # Moran's I
MC.PUD
# rm(glmPUD250)
df250PUD$pearson<-NULL 
df250PUD$response<-NULL

# Run Poisson with smaller random sample (does it remove spatial autocorrelation?)
df250PUD.s<-slice_sample(df250PUD, n = 27315) # n random draws
system.time(glmPUD250.c1<-fit_model(df250PUD.s))
glmPUD250.c1
system.time(glmPUD250.c2<-fit_model(df250PUD.s))
glmPUD250.c2
beta.samples<-mcmc.list(glmPUD250.c1$samples$beta,glmPUD250.c2$samples$beta) # Convergence of Markov chains? https://cran.r-project.org/web/packages/CARBayes/vignettes/CARBayes.pdf pg.22
gelman.diag(beta.samples) # Values less than 1.1, converged
beta.samples.matrix<-rbind(glmPUD250.c1$samples$beta,glmPUD250.c2$samples$beta)
...
df250PUD.s<-cbind(df250PUD.s,glmPUD250.c1$residuals)
df250PUD.s.p<-st_centroid(df250PUD.s)
nb5.PUD.s.p<-knn2nb(knearneigh(df250PUD.s.p, k=5))
nb5.list.PUD.s.p<-nb2listw(nb5.PUD.s.p, style = "B")
MC.PUD<-moran.mc(df250PUD.s$pearson,nb5.list.PUD.s.p,nsim = 999) # Moran's I
MC.PUD
# rm(glmPUD250)
df250PUD.s$pearson<-NULL
df250PUD.s$response<-NULL



# Poisson, spatial correlation modeling (using an arbitrary sample size)
df250PUD.s<-slice_sample(df250PUD, n = 1000) # n random draws
df250PUD.s.p<-st_centroid(df250PUD.s)
...
# plot(nb_soi,coords.df250PUD.s.p)
...
fit_model_CAR<-function(x) S.CARbym(ud ~ RockyShore+SandyBeach+Marshes+Armored+sumpop+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+RESTROOMS+STRS_BEACH+WaterTrail+BayTrail+ChA, data = x, family = "poisson",W=nb.mat.PUD.s.p, burnin=500000, n.sample=3000000, thin=5)
