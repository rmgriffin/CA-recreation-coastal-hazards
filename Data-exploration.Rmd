...
# Packages
PKG <- c("tidyverse","knitr", "renv", "modelsummary", "kableExtra", "corrplot","RColorBrewer","lmtest","sandwich","pscl", "glmmTMB", "broom.mixed","vcd", "lmtest", "sf", "tmap", "tigris", "patchwork", "rnaturalearth", "rgeos", "ggrepel", "ggpubr", "margins") 
...
regs<-function(rs,tipe){ # tipe is either "PUD" or "TUD" / rs is one of {25, 500, 1000}
  models <- list()
  models.rob<-list()
  models[['Poisson']]<-glm(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source==tipe&df$hres==rs,], family = poisson)
  models.rob[['Poisson']]<-coeftest(models[['Poisson']], vcov = sandwich)
  models[['Quasi-Poisson']]<-glm(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source==tipe&df$hres==rs,], family = quasipoisson)
  models.rob[['Quasi-Poisson']]<-coeftest(models[['Quasi-Poisson']], vcov = sandwich)
  models[['Negative Binomial']]<-MASS::glm.nb(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source==tipe&df$hres==rs,])
  models.rob[['Negative Binomial']]<-coeftest(models[['Negative Binomial']], vcov = sandwich)
  models[['Hurdle']]<-hurdle(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source==tipe&df$hres==rs,], dist = "negbin")
  models.rob[['Hurdle']]<-coeftest(models[['Hurdle']], vcov = sandwich)
  models[['ZINB']]<-zeroinfl(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source==tipe&df$hres==rs,], dist = "negbin")
...
PUD250<-regs(250,"PUD")
PUD500<-regs(500,"PUD")
PUD1000<-regs(1000,"PUD")

TUD250<-regs(250,"TUD")
TUD500<-regs(500,"TUD")
TUD1000<-regs(1000,"TUD")
...
bp.2<-cbind(df[df$source=="PUD"&df$hres==250,"ud"],(as.vector(predict(Pmodels[['Quasi-Poisson']],type = "response"))))
...
bp.3<-cbind(df[df$source=="PUD"&df$hres==250,"ud"],(as.vector(predict(Pmodels[['Negative Binomial']],type = "response"))))
...
bp.4<-cbind(df[df$source=="PUD"&df$hres==250,"ud"],(as.vector(predict(Pmodels[['Hurdle']],type = "response"))))
...
bp.5<-cbind(df[df$source=="PUD"&df$hres==250,"ud"],(as.vector(predict(Pmodels[['Zero-inflated Negative Binomial']],type = "response"))))
...
Tmodels <- list()
Tmodels[['Poisson']]<-glm(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source=="TUD"&df$hres==250,], family = poisson) # glmmTMB doesn't converge
Tmodels[['Quasi-Poisson']]<-glmmTMB(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source=="TUD"&df$hres==250,], family = nbinom1)
Tmodels[['Negative Binomial']]<-glmmTMB(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source=="TUD"&df$hres==250,], family = nbinom2)
# Separating the model into two processes for the zeros and the non-zeros does converge, unlike for the Flickr data
Tmodels[['Hurdle']]<-glmmTMB(ud ~ RockyShore + SandyBeach + Marshes + Armored + rdist + sumpop + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source=="TUD"&df$hres==250,], family = "truncated_nbinom2", zi=~.)
Tmodels[['Zero-inflated Negative Binomial']]<-glmmTMB(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source=="TUD"&df$hres==250,], zi = ~., family = nbinom2)
...
msummary(Tmodels[1:3], stars = TRUE, title = "Different regression approaches, Twitter User Days 250m resolution", gof_map = gm, add_rows = rows)

# msummary(Tmodels[4], stars = TRUE, title = "Different regression approaches, Twitter User Days 250m resolution", gof_map = gm)
# 
# msummary(Tmodels[5], stars = TRUE, title = "Different regression approaches, Twitter User Days 250m resolution", gof_map = gm)
...
We have estimated models that treat this as a one (Poisson, Quasi-Poisson, Negative Binomial) or two-stage process (ZINB and hurdle). Based on the log-likelihood and BIC, the hurdle model is the best fit. We do not have a supportive theory for the zero modeling, however. The quasi-poisson model has the best fit of the remaining models, however the rootogram suggests the negative binomial may be the better fit. 

```{r `twitter rootograms`, echo=FALSE}

bp.1<-cbind(df[df$source=="TUD"&df$hres==250,"ud"],(as.vector(predict(Tmodels[['Poisson']],type = "response"))))
...
bp.2<-cbind(df[df$source=="TUD"&df$hres==250,"ud"],(as.vector(predict(Tmodels[['Quasi-Poisson']],type = "response"))))
colnames(bp.2)<-c("obs","pred")
bp.2<-pivot_longer(bp.2, cols = c("obs","pred"), names_to = "status", values_to = "ud")
bp.2$status<-as.factor(bp.2$status)
bp.2$ud<-round(bp.2$ud)
...
bp.3<-cbind(df[df$source=="TUD"&df$hres==250,"ud"],(as.vector(predict(Tmodels[['Negative Binomial']],type = "response"))))
colnames(bp.3)<-c("obs","pred")
bp.3<-pivot_longer(bp.3, cols = c("obs","pred"), names_to = "status", values_to = "ud")
bp.3$status<-as.factor(bp.3$status)
bp.3$ud<-round(bp.3$ud)
...
bp.4<-cbind(df[df$source=="TUD"&df$hres==250,"ud"],(as.vector(predict(Tmodels[['Hurdle']],type = "response"))))
colnames(bp.4)<-c("obs","pred")
bp.4<-pivot_longer(bp.4, cols = c("obs","pred"), names_to = "status", values_to = "ud")
bp.4$status<-as.factor(bp.4$status)
bp.4$ud<-round(bp.4$ud)
...
bp.5<-cbind(df[df$source=="TUD"&df$hres==250,"ud"],(as.vector(predict(Tmodels[['Zero-inflated Negative Binomial']],type = "response"))))
colnames(bp.5)<-c("obs","pred")
bp.5<-pivot_longer(bp.5, cols = c("obs","pred"), names_to = "status", values_to = "ud")
bp.5$status<-as.factor(bp.5$status)
bp.5$ud<-round(bp.5$ud)
...
```{r `standard errors`, echo=FALSE}
# Heteroskedasticity consistent standard errors https://stats.stackexchange.com/questions/117052/replicating-statas-robust-option-in-r
coeftest(Tmodels[['Hurdle']], vcov = vcovHC(Tmodels[['Hurdle']], "HC1"))
coeftest(Tmodels[['Hurdle']], vcov = sandwich)
coeftest(Tmodels[['Hurdle']])

coeftest(Pmodels[[1]], vcov = vcovHC(Pmodels[[1]], "HC1"))

map(Tmodels,~ coeftest(.x, vcov = sandwich))


```
No approach for robust standard erros in R for ZINB estimated by glmmTMB, if needed.
##Draft figures for paper

```{r `regression tree`, echo=FALSE}
# Following https://socviz.co/modeling.html
plot(Pmodels[['Hurdle']], which=2, col=c("red")) # Q-Q plot
