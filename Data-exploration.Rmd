...
# Packages
PKG <- c("tidyverse","knitr", "renv", "modelsummary", "kableExtra", "corrplot","RColorBrewer","lmtest","sandwich","pscl", "glmmTMB", "broom.mixed") 
...
```{r `regressions Poisson`, echo=FALSE, include=FALSE}
models<-list()
...
```{r `regressions quasiPoisson`, echo=FALSE, include=FALSE}
models <- list()
...
msummary(models, stars = TRUE, title = "Quasi-Poisson regression, across data source at 250m")

# coeftest(mod1, vcov = sandwich)
```

Perhaps expand to all resolutions?

```{r `regression NB`, echo=FALSE}
models <- list()
...
msummary(models, stars = TRUE, title = "Negative binomial regression across data source and resolution")

```

```{r `regression comparison flickr`, echo=FALSE}
glmmTMBControl(optCtrl=list(iter.max=1e4,eval.max=1e3))
models <- list()
...
msummary(models, stars = TRUE, title = "Different regression approaches, Flickr User Days 250m resolution")
```
...
msummary(models, stars = TRUE, title = "Different regression approaches, Twitter User Days 250m resolution")
```
