...
coords.df250PUD.s.p<-as.data.frame(st_coordinates(df250PUD.s.p))
df250PUD.s<-cbind(df250PUD.s,coords.df250PUD.s.p)
system.time(fit250PUD<-fitme(ud~RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2+Matern(1|X+Y),data=df250PUD.s,family=poisson()))
plotQQunif(fit250PUD)
plotResiduals(fit250PUD)
testSpatialAutocorrelation(fit250PUD,x=df250PUD.s$X,df250PUD.s$Y)
...
nb.PUD<-dnearneigh(df250PUD.s.p,0,9600)
is.symmetric.nb(nb.PUD)
nb.mat.PUD<-nb2mat(nb.PUD, style = "B", zero.policy = TRUE)
colnames(nb.mat.PUD)<-rownames(nb.mat.PUD)
df250PUD.s$gridcode<-seq(1,nrow(df250PUD.s))
#system.time(fit250PUD<-fitme(ud~RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2+adjacency(1|gridcode),data=df250PUD.s,family=poisson(),adjMatrix=nb.mat.PUD))
system.time(fit250PUDCAR<-HLCor(ud~RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2+adjacency(1|gridcode),data=df250PUD.s,family=poisson(),adjMatrix=nb.mat.PUD))
summary(fit250PUD)
plotQQunif(fit250PUDCAR)
plotResiduals(fit250PUDCAR)
testSpatialAutocorrelation(fit250PUDCAR,x=df250PUD.s$X,df250PUD.s$Y)
