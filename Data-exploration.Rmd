...
# Packages
PKG <- c("tidyverse","knitr", "renv", "modelsummary", "kableExtra", "corrplot","RColorBrewer","lmtest","sandwich","pscl", "glmmTMB", "broom.mixed","vcd", "lmtest", "sf", "tmap") 
...
Rds<-st_read("./Data/Roads_2015.gpkg")
ESI<-st_read("./Data/ESIL_CA.gpkg")
bbox<-filter(Rds, FULLNAME %in% c("Pilarcitos Ave"))
...
yrange <- bbox_new$ymax - bbox_new$ymin
  ...
  bbox_new[3] <- bbox_new[3] + (6 * xrange) # xmax - right
...
ESI<-st_transform(ESI, st_crs(PUD1k))
...
PUD1k.p<-tm_shape(PUD1k,bbox = bbox_new) + 
  tm_polygons(col = "PUD_YR_AVG", breaks = breaks) +
  tm_shape(ESI) +
  tm_lines() 

breaks = c(0, 50, 100, 150, 200)
TUD1k.p<-tm_shape(TUD1k,bbox = bbox_new) + 
  tm_polygons(col = "PUD_YR_AVG", breaks = breaks) +
  tm_shape(ESI) +
  tm_lines() 

plots1k<-tmap_arrange(PUD1k.p,TUD1k.p,ncol = 2)
...
breaks = c(0, 25, 50, 75, 100)
PUD250m.p<-tm_shape(PUD250m,bbox = bbox_new) + 
  tm_polygons(col = "PUD_YR_AVG", breaks = breaks) +
  tm_shape(ESI) +
  tm_lines() 

breaks = c(0, 50, 100, 150, 200)
TUD250m.p<-tm_shape(TUD250m,bbox = bbox_new) + 
  tm_polygons(col = "PUD_YR_AVG", breaks = breaks) +
  tm_shape(ESI) +
  tm_lines() 

plots250m<-tmap_arrange(PUD250m.p,TUD250m.p,ncol = 2)
