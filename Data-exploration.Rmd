...
# Packages
PKG <- c("gstat","tidyverse","knitr", "renv", "modelsummary", "kableExtra", "corrplot","RColorBrewer","lmtest","sandwich","pscl", "glmmTMB", "broom.mixed","vcd", "lmtest", "sf", "tigris", "patchwork", "rnaturalearth", "rgeos", "ggrepel", "margins", "spdep", "shiny", "maptools","glmmfields", "R2BayesX", "BayesXsrc", "coda", "furrr", "hglm", "RANN", "dbscan", "pgirmess", "geoR", "gridExtra", "DHARMa", "remotes", "rcdd","RSpectra") # "adegraphics","ade4","adespatial","spaMM","ggpubr", "CARBayes"
...
# source("./2019Celldata-processing.R") # Produces "DC2019.gpkg" - the cell phone data needed for the next line
# source("./CA-recreation-coastal-hazards.R") # Produces "lresults.csv" and "lresults.gpkg" in next lines, loads data needed for all scripts
#df<-read_csv("lresults.csv") # Results of "CA-recreation-coastal-hazards.R"
df<-st_read("lresults.gpkg")
df<-df %>% dplyr::filter(OA==1) # Only assessing known open access areas
#df<-df %>% dplyr::filter(ud!=0) # Only assessing locations with visitors
```
...
c1<-datasummary((ud == 0) + (ud > 0) ~ (N=1) + Percent(),
            data = df[df$hres==250&df$source=="DC",],fmt="%.0f", title = 'Cellular Resolution 250m')
c2<-datasummary((ud == 0) + (ud > 0) ~ (N=1) + Percent(),
            data = df[df$hres==500&df$source=="DC",],fmt="%.0f", title = 'Cellular Resolution 500m')
c3<-datasummary((ud == 0) + (ud > 0) ~ (N=1) + Percent(),
            data = df[df$hres==1000&df$source=="DC",],fmt="%.0f", title = 'Cellular Resolution 1000m')

knitr::kables(list(c1,c2,c3)) %>%
  kable_styling(position = "center", bootstrap_options = "none")
...
The data is characterized by a significant number of zeros, that increase as a percentage of the data at higher resolutions. Twitter data seems to produce more zeros than flickr or cell data at all resolutions. 

```{r continuous, echo=FALSE}
datasummary((`Distance to nearest road (km)`=rdist ) + (`Population within 8 miles (thousands)`=sumpop) + (`Population within cell (thousands)`=sumpop2) + (`Per capita income`=inc) + (`Coastal vulnerability (1 - 5)`=exposure) + (`Cell networks present`=cell) + (`Distance to nearest wetland (km)`=wtlddist) + (`Annual precipitation (mm/yr)`=meanprec) + (`Air temp (C)`=meanat) + (`Sea surface temp (C)`=meanSST) + (`Chlorophyll a (mg/m3)`=ChA) ~ Mean + Median + SD + Histogram, data = df[df$hres==250,],fmt="%.2f", title = 'Continuous Predictors 250m')

datasummary((`Distance to nearest road (km)`=rdist ) + (`Population within 8 miles (thousands)`=sumpop) + (`Population within cell (thousands)`=sumpop2) + (`Per capita income`=inc) + (`Coastal vulnerability (1 - 5)`=exposure) + (`Cell networks present`=cell) + (`Distance to nearest wetland (km)`=wtlddist) + (`Annual precipitation (mm/yr)`=meanprec) + (`Air temp (C)`=meanat) + (`Sea surface temp (C)`=meanSST) + (`Chlorophyll a (mg/m3)`=ChA) ~ Mean + Median + SD + Histogram, data = df[df$hres==1000,],fmt="%.2f", title = 'Continuous Predictors 1000m')
```

Summary statistics are shown for all potential continuous predictors. There is little variation across resolutions (except pop within cell), but note that the higher resolution observation picks up finer scale variation in the 'distance to X' variables.
...
Correlogram of ESI types. Not much correlation, though sheltered tidal flats and marshes are often observed together.

```{r correlationYC, echo=FALSE, fig.align="center"}
cormat<-df.nosf[df.nosf$hres==250,] %>%
  dplyr::select(inc,cell,exposure,sumpop,sumpop2,rdist,BIKE_PATH,BOATING,BT_FACIL_T,CAMPGROUND,DSABLDACSS,FEE,FISHING,PARKING,PTH_BEACH,RESTROOMS,STRS_BEACH,WaterTrail,BayTrail,wtlddist,meanprec,meanat,meanSST,ChA,RockyShore,Armored,Marshes,SandyBeach,TidalFlat)
...
df %>% mutate(armor = ifelse(Armored>0,"Armored","Non-armored")) %>% 
  #as.factor(armor) %>% 
  st_drop_geometry() %>% 
  group_by(armor, hres) %>% 
  summarise(Vulnerability = mean(exposure), n = length(exposure), CellPop = mean(sumpop))
```

Correlogram of other predictors. Relationships are stable across resolutions; the above plots correlations at 250m resolution. Strong correlations (>|0.5|) are observed between:
- Mean air temp and Pop within 8 miles (+)
- Road distance and marsh distance (+)
- Road distance and ChA (-)
- Fee and campground (+)
- Boating facility and boating (+)
- Parking and disabled access (+)
- Parking and fishing (+)
- Restrooms and disabled access (+)
- Restrooms and fee (+)
- Restrooms and fishing (+)
- Restrooms and parking (+)
- Marsh distance and ChA (-)
- Mean prec and mean air temp (-)
- Mean prec and mean sea surface temp (-)
- Mean air temp and mean sea surface temp (+)
- Tidal flat and marshes

Should consider removing: mean prec, restrooms, mean sea surface temp, boating

```{r `ESI crosswalk`, echo=FALSE}
d1<-datasummary((`Rocky shore`=RockyShore) + (`Sandy beach`=SandyBeach) + (`Marshes`=Marshes) + (`Armored`=Armored) + (`TidalFlat`=TidalFlat) ~ Mean + SD, data = df[df$hres==1000,],fmt="%.2f", title = 'ESI Length (m) @ 1000m')
d2<-datasummary((`Rocky shore`=RockyShore) + (`Sandy beach`=SandyBeach) + (`Marshes`=Marshes) + (`Armored`=Armored) + (`TidalFlat`=TidalFlat) ~ Mean + SD, data = df[df$hres==500,],fmt="%.2f", title = 'ESI Length (m) @ 500m')
d3<-datasummary((`Rocky shore`=RockyShore) + (`Sandy beach`=SandyBeach) + (`Marshes`=Marshes) + (`Armored`=Armored) + (`TidalFlat`=TidalFlat) ~ Mean + SD, data = df[df$hres==250,],fmt="%.2f", title = 'ESI Length (m) @ 250m')
...
```{r `ESI crosswalk2`, echo=FALSE}
d1<-datasummary(ud ~ Mean*source, data = df[df$hres==250&df$Marshes>0,],fmt="%.2f", title = 'Marshes')
d2<-datasummary(ud ~ Mean*source, data = df[df$hres==250&df$RockyShore>0,],fmt="%.2f", title = 'Rocky Shore')
d3<-datasummary(ud ~ Mean*source, data = df[df$hres==250&df$SandyBeach>0,],fmt="%.2f", title = 'Sandy Beach')
d4<-datasummary(ud ~ Mean*source, data = df[df$hres==250&df$Armored>0,],fmt="%.2f", title = 'Armored')
d5<-datasummary(ud ~ Mean*source, data = df[df$hres==250&df$TidalFlat>0,],fmt="%.2f", title = 'Tidal Flat')

knitr::kables(list(d1,d2,d3,d4,d5)) %>%
  kable_styling(position = "center", bootstrap_options = "none")
```
User days by shoreline ESI class

```{r `Adding variables`, echo=FALSE}
## Adding variables
# Do not transform count data DOI: https://doi.org/10.1111/j.2041-210X.2010.00021.x
# For testing the significance of regression coefficients, go ahead and log‐transform count data https://doi.org/10.1111/2041-210X.12386
...
df$meanat2<-df$meanat^2 # Polynomial air temp
df$RockyShoreI<-ifelse(df$RockyShore==0,0,1) # Indicator variables instead of lengths, presence/absence in a cell
df$SandyBeachI<-ifelse(df$SandyBeach==0,0,1)
df$MarshesI<-ifelse(df$Marshes==0,0,1)
df$ArmoredI<-ifelse(df$Armored==0,0,1)
df$TidalFlatI<-ifelse(df$TidalFlat==0,0,1)
df$x3aI<-ifelse(df$x3a==0,0,1)
df$x10aI<-ifelse(df$x10a==0,0,1)
```

```{r `Dep Var regressions`, echo=FALSE}
# regs<-function(rs,tipe,dv){ # tipe is either "PUD","TUD","DC" / rs is one of {25, 500, 1000} / dv is either "ud" or "ihsud" or "lnud"
#   models <- list()
#   models.rob<-list()
...
# Don't bother with this https://twitter.com/jmwooldridge/status/1388249554399731712
...
```{r `glm Poisson`, echo=FALSE}
## Regression across observation size

Pois.reg<-function(tipe,dv,v){ # tipe is either "PUD","TUD","DC" / dv is either "ud" or "ihsud" or "lnud" / v is a list of variables to include, referenced by column name
  models <- list()
  models.rob<-list()
  models[['p250']]<-glm(reformulate(termlabels = v,response = dv), data = df[df$source==tipe&df$hres==250,], family = poisson)
  models.rob[['p250']]<-coeftest(models[['p250']], vcov = sandwich)
  
  models[['p500']]<-glm(reformulate(termlabels = v,response = dv), data = df[df$source==tipe&df$hres==500,], family = poisson)
  models.rob[['p500']]<-coeftest(models[['p500']], vcov = sandwich)
  
  models[['p1000']]<-glm(reformulate(termlabels = v,response = dv), data = df[df$source==tipe&df$hres==1000,], family = poisson)
  models.rob[['p1000']]<-coeftest(models[['p1000']], vcov = sandwich)
...
PUD.p<-Pois.reg("PUD","ud",varlistI)
PUD.p[[2]]
TUD.p<-Pois.reg("TUD","ud",varlistI)
TUD.p[[2]]
DC.p<-Pois.reg("DC","ud",varlistI)
DC.p[[2]]

a<-c(PUD.p[[2]],TUD.p[[2]],DC.p[[2]])
names(a)<-c("PUD250m","PUD500m","PUD1000m","TUD250m","TUD500m","TUD1000m","DC250m","DC500m","DC1000m")

msummary(a, stars = TRUE)

## Regression using 250m observation size, estimating spatial autocorrelation

set.seed(1218)

s<-1 # sample proportion, used to check for spatial autocorrelation in smaller samples. Set to 1 to use the whole sample.
dfs<-slice_sample(df, prop = s)

PUD.p<-glm(reformulate(termlabels = varlist, response = "ud"), data = dfs[dfs$source=="PUD"&dfs$hres==250,], family = poisson)
TUD.p<-glm(reformulate(termlabels = varlist, response = "ud"), data = dfs[dfs$source=="TUD"&dfs$hres==250,], family = poisson)
DC.p<-glm(reformulate(termlabels = varlist, response = "ud"), data = dfs[dfs$source=="DC"&dfs$hres==250,], family = poisson)
...
df250PUD<-dfs[dfs$source=="PUD"&dfs$hres==250,]
df250PUD$PUDres<-resid(PUD.p)
#st_write(df250PUD,"df250PUD.gpkg") # Exporting to view in QGIS

df250TUD<-dfs[dfs$source=="TUD"&dfs$hres==250,]
df250TUD$TUDres<-resid(TUD.p)
#st_write(df250TUD,"df250TUD.gpkg") # Exporting to view in QGIS

df250DC<-dfs[dfs$source=="DC"&dfs$hres==250,]
df250DC$DCres<-resid(DC.p)
#st_write(df250DC,"df250DC.gpkg") # Exporting to view in QGIS

# # Moran's I calculation https://cran.r-project.org/web/packages/adespatial/vignettes/tutorial.html https://rpubs.com/FelipeSantos/LISA_tmap_geoda https://cran.r-project.org/web/packages/spdep/vignettes/nb_sf.html
# nb.PUD<-poly2nb(df250PUD)
# nb.list.PUD<-nb2listw(nb.PUD, style = "B", zero.policy = TRUE) 
# print(nb.list.PUD, zero.policy=TRUE) # https://github.com/r-spatial/spdep/issues/45
# print(listw2mat(nb.list.PUD)[1:10, 1:10], digits = 3)
# MC.PUD<-moran.mc(df250PUD$PUDres,nb.list.PUD,nsim = 999,zero.policy = TRUE)
# MC.PUD
# 
# nb.TUD<-poly2nb(df250TUD)
# nb.list.TUD<-nb2listw(nb.TUD, zero.policy = TRUE)
# MC.TUD<-moran.mc(df250TUD$TUDres,nb.list.TUD,nsim = 999,zero.policy = TRUE)
# MC.TUD
# 
# nb.DC<-poly2nb(df250DC)
# nb.list.DC<-nb2listw(nb.DC, zero.policy = TRUE)
# MC.DC<-moran.mc(df250DC$DCres,nb.list.DC,nsim = 999,zero.policy = TRUE) # https://rspatial.org/raster/analysis/analysis.pdf (pg.27)
# MC.DC
# moran.test(df250DC$DCres,nb.list.DC,zero.policy = TRUE)
# moran.plot(df250DC$DCres,nb.list.DC,zero.policy = TRUE, labels = FALSE)

# Moran I using # nearest neighbors, versus adjacent neighbors, following guidance in Kelly (2020), pg. 11-12 https://www.ucd.ie/economics/t4media/WP20_06.pdf
df250PUD.p<-st_centroid(df250PUD) # K nearest neighbors only accepts points
nb.PUD<-knn2nb(knearneigh(df250PUD.p, k=20))
nb.list.PUD.p<-nb2listw(nb.PUD, style = "B", zero.policy = TRUE)
print(nb.list.PUD.p, zero.policy=TRUE) # https://github.com/r-spatial/spdep/issues/45
print(listw2mat(nb.list.PUD.p)[1:10, 1:10], digits = 3)
#MC.PUD.p<-moran.mc(df250PUD.p$PUDres,nb.list.PUD.p,nsim = 999,zero.policy = TRUE) # Do not use for regression residuals http://www.sethspielman.org/courses/geog5023/lectures/Lecture_8_2015.pdf
MC.PUD.p<-lm.morantest(PUD.p,nb.list.PUD.p)
MC.PUD.p

df250TUD.p<-st_centroid(df250TUD)
nb.TUD<-knn2nb(knearneigh(df250TUD.p, k=20))
nb.list.TUD.p<-nb2listw(nb.TUD, style = "B", zero.policy = TRUE)
#MC.TUD.p<-moran.mc(df250TUD.p$TUDres,nb.list.TUD.p,nsim = 999,zero.policy = TRUE)
MC.TUD.p<-lm.morantest(TUD.p,nb.list.TUD.p)
MC.TUD.p

df250DC.p<-st_centroid(df250DC)
nb.DC<-knn2nb(knearneigh(df250DC.p, k=20))
nb.list.DC.p<-nb2listw(nb.DC, style = "B", zero.policy = TRUE)
#MC.DC.p<-moran.mc(df250DC.p$DCres,nb.list.DC.p,nsim = 999,zero.policy = TRUE)
MC.DC.p<-lm.morantest(DC.p,nb.list.DC.p)
MC.DC.p

# Correlograms
system.time(corPUD250<-spdep::sp.correlogram(nb.PUD,df250PUD.p$PUDres,order=20,method = "I", zero.policy = TRUE))
#plot(corPUD250)
system.time(corTUD250<-spdep::sp.correlogram(nb.TUD,df250TUD.p$TUDres,order=20,method = "I", zero.policy = TRUE))
system.time(corDC250<-spdep::sp.correlogram(nb.DC,df250DC.p$DCres,order=20,method = "I", zero.policy = TRUE))
plot(corPUD250)
plot(corTUD250)
plot(corDC250)

# Variogram https://www.aspexit.com/implementing-variograms-in-r/ https://r-spatial.org/r/2016/02/14/gstat-variogram-fitting.html 
df250PUD.p<-as_Spatial(df250PUD.p)
df250TUD.p<-as_Spatial(df250TUD.p)
df250DC.p<-as_Spatial(df250DC.p)

varPUD250<-gstat::variogram(PUDres~1, data = df250PUD.p, cutoff = 15000)
varTUD250<-gstat::variogram(TUDres~1, data = df250TUD.p, cutoff = 15000)
varDC250<-gstat::variogram(DCres~1, data = df250DC.p, cutoff = 15000)

varPUD250fit<-gstat::fit.variogram(varPUD250, model = vgm("Exp"))
varPUD250fit

varTUD250fit<-gstat::fit.variogram(varTUD250, model = vgm("Exp"))
varTUD250fit

varDC250fit<-gstat::fit.variogram(varDC250, model = vgm("Exp"))
varDC250fit

plot(varPUD250,varPUD250fit)
plot(varTUD250,varTUD250fit)
plot(varDC250,varDC250fit)

```
Range of 9600 for PUD, 8200 for TUD, 800 for DC (meters)
```{r `OLS version, including cell`, echo=FALSE}
# OLS.reg<-function(tipe,dv,v){ # tipe is either "PUD","TUD","DC" / dv is either "ud" or "ihsud" or "lnud" / v is a list of variables to include, referenced by column name
#   models <- list()
#   models.rob<-list()
#   models[['p250']]<-lm(reformulate(termlabels = v,response = dv), data = df[df$source==tipe&df$hres==250,])
#   models.rob[['p250']]<-coeftest(models[['p250']], vcov = sandwich)
#   
#   models[['p500']]<-lm(reformulate(termlabels = v,response = dv), data = df[df$source==tipe&df$hres==500,])
#   models.rob[['p500']]<-coeftest(models[['p500']], vcov = sandwich)
#   
#   models[['p1000']]<-lm(reformulate(termlabels = v,response = dv), data = df[df$source==tipe&df$hres==1000,])
#   models.rob[['p1000']]<-coeftest(models[['p1000']], vcov = sandwich)
# 
#   return(list(models,models.rob))
# }
# 
# PUD.o<-OLS.reg("PUD","ud",varlist)
# PUD.o[[2]]
# TUD.o<-OLS.reg("TUD","ud",varlist)
# TUD.o[[2]]
# DC.o<-OLS.reg("DC","ud",varlist)
# DC.o[[2]]
# 
# b<-c(PUD.o[[2]],TUD.o[[2]],DC.o[[2]])
# names(b)<-c("PUD250m","PUD500m","PUD1000m","TUD250m","TUD500m","TUD1000m","DC250m","DC500m","DC1000m")
# 
# msummary(b, stars = TRUE)
# rm(b)
```

```{r `OLS and Poisson`, echo=FALSE}
# c<-list()
# c[["OLS_PUD"]]<-PUD.o[[2]]$p250
# c[["OLS_TUD"]]<-TUD.o[[2]]$p250
# c[["OLS_DC"]]<-DC.o[[2]]$p250
# c[["Poisson_PUD"]]<-PUD.p[[2]]$p250
# c[["Poisson_TUD"]]<-TUD.p[[2]]$p250
# c[["Poisson_DC"]]<-DC.p[[2]]$p250
# 
# msummary(c, stars = TRUE)
# rm(c)
```

```{r `GLMMfields Poisson`, echo=FALSE}
# #options(mc.cores = parallel::detectCores())
# df250PUD.p<-cbind(df250PUD.p,st_coordinates(df250PUD.p$geom)) 
# system.time(m_spatial<-glmmfields(ud ~ RockyShore+SandyBeach+Marshes+Armored+sumpop+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+RESTROOMS+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2, data = df250PUD.p, family = poisson(link = "log"), lat = "X", lon = "Y"))
# df250PUD.phat<-predict(m_spatial, type = "response")
# df250PUD.p$phat<-df250PUD.phat$estimate
# df250PUD.p$PUDres2<-df250PUD.p$phat-df250PUD.p$ud
# st_write(df250PUD.p,"df250PUDp.gpkg") # Exporting to view in QGIS 
# MC.PUD.p<-moran.mc(df250PUD.p$PUDres2,nb.list.PUD.p,nsim = 999,zero.policy = TRUE)
# MC.PUD.p # Same spatial autocorrelation problem as running without random fields
```

```{r `CARBayes Poisson`, echo=FALSE}
# Poisson, no spatial correlation modeling
# sort(sapply(ls(), function(x) format(object.size(get(x)), unit = 'auto'))) # Memory used by objects, to determine how many workers are possible
df250PUD<-df[df$source=="PUD"&df$hres==250,]
df250TUD<-df[df$source=="TUD"&df$hres==250,]
df250DC<-df[df$source=="DC"&df$hres==250,]

fit_model<-function(x) S.glm(ud ~ RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+BOATING+CAMPGROUND+DSABLDACSS+FEE+FISHING+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2, data = x, family = "poisson", burnin=10000, n.sample=200000, thin=5)
system.time(glmPUD250.c1<-fit_model(df250PUD))
glmPUD250.c1
gc()
system.time(glmPUD250.c2<-fit_model(df250PUD))
glmPUD250.c2
gc()
beta.samples<-mcmc.list(glmPUD250.c1$samples$beta,glmPUD250.c2$samples$beta) # Convergence of Markov chains? https://cran.r-project.org/web/packages/CARBayes/vignettes/CARBayes.pdf pg.22
gelman.diag(beta.samples) # Values less than 1.1, converged
beta.samples.matrix<-rbind(glmPUD250.c1$samples$beta,glmPUD250.c2$samples$beta)
colnames(beta.samples.matrix)<-colnames(glmPUD250.c1$X)
round(t(apply(beta.samples.matrix, 2, quantile, c(0.5, 0.025, 0.975))),5) # inference

# Moran's I
df250PUD<-cbind(df250PUD,glmPUD250.c1$residuals)
df250PUD.p<-st_centroid(df250PUD) # K nearest neighbors only accepts points
nb.PUD<-knn2nb(knearneigh(df250PUD.p, k=20))
nb.list.PUD.p<-nb2listw(nb.PUD, style = "B", zero.policy = TRUE)
print(nb.list.PUD.p, zero.policy=TRUE) # https://github.com/r-spatial/spdep/issues/45
print(listw2mat(nb.list.PUD.p)[1:10, 1:10], digits = 3)
MC.PUD.p<-moran.mc(df250PUD.p$pearson,nb.list.PUD.p,nsim = 999,zero.policy = TRUE) # can't use lm.morantest as S.glm does not return lm objects
MC.PUD.p
# rm(glmPUD250)
df250PUD$pearson<-NULL 
df250PUD$response<-NULL
df250PUD.p$pearson<-NULL 
df250PUD.p$response<-NULL

# Poisson, spatial correlation modeling (using an arbitrary sample proportion, p)
p<-.25
df250PUD.s<-slice_sample(df250PUD, prop = p) # proportion of random draws from sample
df250PUD.s.p<-st_centroid(df250PUD.s)
coords.df250PUD.s.p<-st_centroid(st_geometry(df250PUD.s.p))

nb.PUD.s.p<-knn2nb(knearneigh(df250PUD.s.p, k=round((9600/250)*p)), sym = TRUE) # k formula dynamically adapts neighbor calculation based on sample proportion used in CAR and known range from variogram
plot(nb.PUD.s.p,coords.df250PUD.s.p)
nb.mat.PUD.s.p<-nb2mat(nb.PUD.s.p, style = "B", zero.policy = NULL)

# trinb.PUD.s.p<-tri2nb(coords.df250PUD.s.p) # This approach will create symmetric neighbors, using sphere of influence approach
# nbsoi.PUD.s.p<-graph2nb(soi.graph(trinb.PUD.s.p, coords.df250PUD.s.p), sym=TRUE)
# # plot(nb_soi.PUD.s.p,coords.df250PUD.s.p)
# nb.mat.PUD.s.p<-nb2mat(nbsoi.PUD.s.p, style = "B", zero.policy = NULL)
# colnames(nb.mat.PUD.s.p)<-rownames(nb.mat.PUD.s.p) # https://stackoverflow.com/questions/27304797/spatial-weights-asymmetric-adjacency-matrix
# isSymmetric(nb.mat.PUD.s.p)

fit_model_CAR<-function(x) S.CARbym(ud ~ RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2, data = x, family = "poisson",W=nb.mat.PUD.s.p, burnin=500000, n.sample=3000000, thin=5)
#fit_model_CAR<-function(x) S.CARbym(ud ~ RockyShore+SandyBeach+Marshes+Armored+sumpop+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+RESTROOMS+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2, data = x, family = "poisson",W=nb.mat.PUD.s.p, burnin=500000, n.sample=2200000, thin=1)

system.time(glmPUD250CAR.1<-fit_model_CAR(df250PUD.s))
rPUD250c1<-glmPUD250CAR.1$residuals
c1<-glmPUD250CAR.1$samples$beta
rm(glmPUD250CAR.1)
gc()

system.time(glmPUD250CAR.2<-fit_model_CAR(df250PUD.s))
c2<-glmPUD250CAR.2$samples$beta
rm(glmPUD250CAR.2)
gc()

system.time(glmPUD250CAR.3<-fit_model_CAR(df250PUD.s))
c3<-glmPUD250CAR.3$samples$beta
rm(glmPUD250CAR.3)
gc()

system.time(glmPUD250CAR.4<-fit_model_CAR(df250PUD.s))
c4<-glmPUD250CAR.4$samples$beta
rm(glmPUD250CAR.4)
gc()

system.time(glmPUD250CAR.5<-fit_model_CAR(df250PUD.s))
c5<-glmPUD250CAR.5$samples$beta
rm(glmPUD250CAR.5)
gc()

# Model diagnostics
beta.samples<-mcmc.list(c1,c2)
gelman.diag(beta.samples)
beta.samples<-mcmc.list(c1,c2,c3)
gelman.diag(beta.samples)
beta.samples<-mcmc.list(c1,c2,c3,c4)
gelman.diag(beta.samples)
beta.samples<-mcmc.list(c1,c2,c3,c4,c5)
gelman.diag(beta.samples)
# https://cran.r-project.org/web/packages/CARBayes/vignettes/CARBayes.pdf pg.22
traceplot(c5[,6], smooth = TRUE) # Issues with convergence? https://m-clark.github.io/bayesian-basics/diagnostics.html

# Moran's I
df250PUD.s<-cbind(df250PUD.s,rPUD250c1)
df250PUD.s.p<-st_centroid(df250PUD.s) # K nearest neighbors only accepts points
nb.PUD<-knn2nb(knearneigh(df250PUD.s.p, k=round((9600/250)*p)))
nb.list.PUD.p<-nb2listw(nb.PUD, style = "B", zero.policy = TRUE)
print(nb.list.PUD.p, zero.policy=TRUE) # https://github.com/r-spatial/spdep/issues/45
print(listw2mat(nb.list.PUD.p)[1:10, 1:10], digits = 3)
MC.PUD.p<-moran.mc(df250PUD.s$pearson,nb.list.PUD.p,nsim = 999,zero.policy = TRUE) # can't use lm.morantest as S.glm does not return lm objects
MC.PUD.p
```

```{r `spaMM`, echo=FALSE}

#Function variables: src {"PUD","TUD","DC"}, p {0>p<=1} an arbitrary sample proportion, 

df250<-df[df$source=="PUD"&df$hres==250,]
p<-.05

# Poisson, spatial correlation modeling
df250.s<-slice_sample(df250, prop = p) # proportion of random draws from sample
df250.s.p<-st_centroid(df250.s)

# Regular glm, no spatial autocorrelation
fit250GLM<-glm(ud~RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2,data=df250.s,family="poisson")
summary(fit250GLM)

#plotQQunif(fit250GLM)
#plotResiduals(fit250GLM)
coords.df250.s.p<-as.data.frame(st_coordinates(df250.s.p))
df250.s<-cbind(df250.s,coords.df250.s.p)
testSpatialAutocorrelation(fit250GLM,x=df250.s$X,df250.s$Y)

df250.s$GLMres<-residuals(fit250GLM)

df250.sp<-as_Spatial(df250.s)

var250GLM<-gstat::variogram(GLMres~1, data = df250.sp, cutoff = 20000, width = 250)
plot(var250GLM)

var250GLMfit<-gstat::fit.variogram(var250GLM, vgm(c("Nug","Exp", "Mat", "Sph", "Gau", "Ste", "Bes", "Pen", "Wav"))) # fits a model, choosing best - only using models that converge from vgm() list
range<-var250GLMfit[2,3]
plot(var250GLM, var250GLMfit)

#show.vgms()

# # Conditional autoregressive
# #nb.PUD<-knn2nb(knearneigh(df250PUD.s.p, k=round((9600/250)*p)), sym = TRUE) # k formula dynamically adapts neighbor calculation based on sample proportion used in CAR and known range from variogram
# nb<-dnearneigh(df250.s.p, d1=0,d2=range)
# is.symmetric.nb(nb)
# nb.mat<-nb2mat(nb, style = "B", zero.policy = TRUE)
# colnames(nb.mat)<-rownames(nb.mat)
# df250.s$gridcode<-seq(1,nrow(df250.s))
# system.time(fit250CAR<-fitme(ud~RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2+adjacency(1|gridcode),data=df250.s,family=poisson(),adjMatrix=nb.mat))
# #system.time(fit250CAR<-HLCor(ud~RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2+adjacency(1|gridcode),data=df250.s,family=poisson(),adjMatrix=nb.mat)) # Slow for large datasets compared to "fitme", see pg 12 https://cran.r-project.org/web/packages/spaMM/spaMM.pdf
# summary(fit250CAR)
# plotQQunif(fit250CAR)
# plotResiduals(fit250CAR)
# testSpatialAutocorrelation(fit250CAR,x=df250.s$X,df250.s$Y)
# 
# df250.s$CARres<-residuals(fit250CAR)
# 
# df250.sp<-as_Spatial(df250.s)
# 
# var250CAR<-gstat::variogram(CARres~1, data = df250.sp, cutoff = 20000)
# plot(var250CAR)
# 
# var250CARfit<-gstat::fit.variogram(var250CAR, vgm(c("Nug", "Exp", "Mat", "Sph")))
# var250CARfit

# Matern random effect
system.time(fit250M<-fitme(ud~RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2+Matern(1|X+Y),data=df250.s, fixed=list(nu=0.5), family=poisson()))
summary(fit250M)
plotQQunif(fit250M)
plotResiduals(fit250M)
testSpatialAutocorrelation(fit250M,x=df250.s$X,df250.s$Y)
df250.s$Mres<-residuals(fit250M)
df250.sp<-as_Spatial(df250.s)

var250M<-gstat::variogram(Mres~1, data = df250.sp, cutoff = 20000, width = 500)
plot(var250M)

var250Mfit<-gstat::fit.variogram(var250M, vgm(c("Nug", "Exp", "Mat", "Sph")))
var250Mfit
plot(var250M, var250Mfit)

# # Negative binomial
# system.time(fit250CARNB<-fitme(ud~RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2+adjacency(1|gridcode),data=df250.s,family=spaMM::negbin(shape = 1),adjMatrix=nb.mat))
# summary(fit250CARNB)
# plotQQunif(fit250CARNB)
# plotResiduals(fit250CARNB)
# testSpatialAutocorrelation(fit250CARNB,x=df250.s$X,df250.s$Y)
# 
# df250.s$CARres<-residuals(fit250CARNB)
# 
# df250.sp<-as_Spatial(df250.s)
# 
# var250CARNB<-gstat::variogram(CARres~1, data = df250.sp, cutoff = 20000)
# plot(var250CARNB)
# 
# var250CARNBfit<-gstat::fit.variogram(var250CARNB, vgm(c("Nug", "Exp", "Mat", "Sph")))
# var250CARNBfit
```

```{r `HGLM`, echo=FALSE}
# # Need a symmetric matrix and cannot have rowsum == 0, therefore need to remove observations with no neighbors
# `%notin%` <- Negate(`%in%`) # https://www.marsja.se/how-to-use-in-in-r/
# df250PUD<-filter(df250PUD, id %notin% c(15496, 16627, 34043, 72159, 72160, 121144, 172721, 245112, 249054, 278856, 286903, 286902, 289417))
# nb.PUD<-poly2nb(df250PUD)
# nb.lw<-nb2listw(nb.PUD)
# nb.W<-as(nb.lw, "CsparseMatrix")
# df250PUD$index<-seq(1,length(df250PUD$id),1)
# 
# system.time(HGLM_sar<-hglm(fixed=ud ~ RockyShore+SandyBeach+Marshes+Armored+sumpop+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+RESTROOMS+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2, random= ~ 1|index, data=df250PUD, family=poisson(link=log), rand.family=SAR(D=nb.W)))
# 
# system.time(HGLM_car<-hglm(fixed=ud ~ RockyShore+SandyBeach+Marshes+Armored+sumpop+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+RESTROOMS+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2, random= ~ 1|index, data=df250PUD, family=poisson(link=log), rand.family=CAR(D=nb.W)))
```

```{r `R2BayesX`, echo=FALSE}
# # Need a symmetric matrix and cannot have rowsum == 0, therefore need to remove observations with no neighbors 
# `%notin%` <- Negate(`%in%`) # https://www.marsja.se/how-to-use-in-in-r/
# df250PUD<-filter(df250PUD, id %notin% c(15496, 16627, 34043, 72159, 72160, 121144, 172721, 245112, 249054, 278856, 286903, 286902, 289417))
# nb.PUD<-poly2nb(df250PUD, queen = TRUE)
# nb.PUD # See printout for rows in the dataframe that have no links, if any
# is.symmetric.nb(nb.PUD)
# n.comp.nb(nb.PUD)$nc # https://edzer.github.io/UseR2019/part2.html 58 distinct graph components, due to islands. Not allowed with R2BayesX
# comps<-n.comp.nb(nb.PUD)
# hist(comps$comp.id)
# 
# nb.bx.PUD<-nb2gra(nb.PUD)
# nb.bx.PUD.struct<-as.numeric(rownames(nb.bx.PUD))
# df250PUD$r2bx<-nb.bx.PUD.struct
# formulaR2BX<-ud ~ sx(RockyShore) + sx(SandyBeach) + sx(Marshes) + sx(Armored) + sx(sumpop) + sx(rdist) + sx(BIKE_PATH) + sx(CAMPGROUND) + sx(DSABLDACSS) + sx(FEE) + sx(PARKING) + sx(PTH_BEACH) + sx(RESTROOMS) + sx(STRS_BEACH) + sx(WaterTrail) + sx(BayTrail) + sx(ChA) + sx(meanat) + sx(meanat2) + sx(r2bx, bs = "spatial", map = nb.bx.PUD) 
# system.time(b<-bayesx(formula = formulaR2BX, family = "poisson", method = "MCMC", data = df250PUD))
# # bayesx_logfile(b)
```



```{r `Negative binomial version, including cell`, echo=FALSE}
# NB.reg<-function(tipe,dv){ # tipe is either "PUD","TUD","DC" / rs is one of {25, 500, 1000} / dv is either "ud" or "ihsud" or "lnud"
#   models <- list()
#   models.rob<-list()
# 
#   models[['p250']]<-MASS::glm.nb(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","FISHING","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat"),response = dv), data = df[df$source==tipe&df$hres==250,])
#   models.rob[['p250']]<-coeftest(models[['p250']], vcov = sandwich)
#   
#   models[['p500']]<-MASS::glm.nb(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","FISHING","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat"),response = dv), data = df[df$source==tipe&df$hres==500,])
#   models.rob[['p500']]<-coeftest(models[['p500']], vcov = sandwich)
#   
#   models[['p1000']]<-MASS::glm.nb(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","CAMPGROUND","DSABLDACSS","FEE","FISHING","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","ChA","meanat"),response = dv), data = df[df$source==tipe&df$hres==1000,])
#   models.rob[['p1000']]<-coeftest(models[['p1000']], vcov = sandwich)
# 
#   return(list(models,models.rob))
# }
# 
# PUD.nb<-NB.reg("PUD","ud")
# PUD.nb[[2]]
# TUD.nb<-NB.reg("TUD","ud")
# TUD.nb[[2]]
# DC.nb<-NB.reg("DC","ud")
# DC.nb[[2]]
# 
# e<-c(coef(a[["DC250m"]]))
# 
# MASS::glm.nb(reformulate(termlabels = c("RockyShore","SandyBeach","Marshes","Armored","sumpop","rdist","BIKE_PATH","BOATING","BT_FACIL_T","CAMPGROUND","DSABLDACSS","FEE","FISHING","PARKING","PTH_BEACH","RESTROOMS","STRS_BEACH","WaterTrail","BayTrail","wtlddist","meanprec","meanat","meanSST"),response = "ud"), data = df[df$source=="DC"&df$hres==250,], start = e, control=glm.control(trace=20,maxit=100))
# 
# 
#   models.rob[['p250']]<-coeftest(models[['p250']], vcov = sandwich)
# 
...
ME(ud ~ RockyShore + SandyBeach + Marshes + Armored + sumpop + rdist + BIKE_PATH + BOATING + BT_FACIL_T + CAMPGROUND + DSABLDACSS + FEE + FISHING + PARKING + PTH_BEACH + RESTROOMS + STRS_BEACH + WaterTrail + BayTrail + wtlddist + meanprec + meanat + meanSST, data = df[df$source=="PUD"&df$hres==1000,], family = "poisson", listw = lnP1000)
...
```{r `spatial regression`, echo=FALSE}
