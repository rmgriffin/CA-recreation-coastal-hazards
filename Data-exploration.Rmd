...
a<-ggplot() +
  ...
  theme(legend.position = c(0.25, 0.3), legend.margin = margin(-0.25,0,0,0, unit="cm"), panel.background = element_rect(fill = "aliceblue", color = NA), axis.title.x=element_blank(), axis.title.y=element_blank())
...
b<-ggplot() +
  ...
  geom_sf_label(data = cdp, aes(label=NAME), size=2.5) +
  coord_sf(xlim=c(-122.5402,-122.3882),ylim=c(37.38338,37.5586), datum = NA) +
  theme(legend.position = c(0.25, 0.3), panel.background = element_rect(fill = "aliceblue", color = NA), axis.title.x=element_blank(), axis.title.y=element_blank())
...
c<-ggplot() +
  ...
  geom_sf(data=ESI_bbox, aes(color=I1), lwd = 2, show.legend = TRUE) +
  scale_color_manual(values = coler1) +
  geom_sf(data=ESI_bbox, aes(color=I2), lwd = 1, show.legend = FALSE) +
  scale_color_manual(values = coler1, name = "ESI Classes", labels = c("Exposed solid man-made structures", "Exposed wave-cut platforms in bedrock", "Fine-to-medium grain sand beaches", "Coarse-grained sand beaches", "Mixed sand and gravel beaches", "Riprap", "Sheltered man-made structures", "Sheltered riprap")) +
  coord_sf(xlim=c(-122.5168,-122.4233),ylim=c(37.49121,37.5148), datum = NA) +
  theme(legend.position = c(.85, 0.7), panel.background = element_rect(fill = "aliceblue", color = NA), axis.title.x=element_blank(), axis.title.y=element_blank())
