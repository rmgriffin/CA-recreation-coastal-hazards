...
fit_model<-function(x) S.glm(ud ~ RockyShore+SandyBeach+Marshes+Armored+TidalFlat+sumpop+sumpop2+rdist+BIKE_PATH+CAMPGROUND+DSABLDACSS+FEE+PARKING+PTH_BEACH+STRS_BEACH+WaterTrail+BayTrail+ChA+meanat+meanat2, data = x, family = "poisson", burnin=10000, n.sample=200000, thin=5)
...
df250PUD.p<-st_centroid(df250PUD) # K nearest neighbors only accepts points
nb5.PUD<-knn2nb(knearneigh(df250PUD.p, k=5))
nb.list.PUD.p<-nb2listw(nb5.PUD, style = "B", zero.policy = TRUE)
print(nb.list.PUD.p, zero.policy=TRUE) # https://github.com/r-spatial/spdep/issues/45
print(listw2mat(nb.list.PUD.p)[1:10, 1:10], digits = 3)
MC.PUD.p<-moran.mc(df250PUD.p$pearson,nb.list.PUD.p,nsim = 999,zero.policy = TRUE)
MC.PUD.p
# rm(glmPUD250)
...
# Poisson, spatial correlation modeling (using an arbitrary sample size)
df250PUD.s<-slice_sample(df250PUD, n = 5000) # n random draws
df250PUD.s.p<-st_centroid(df250PUD.s)
coords.df250PUD.s.p<-st_centroid(st_geometry(df250PUD.s.p))

# nb5.PUD.s.p<-knn2nb(knearneigh(df250PUD.s.p, k=2), sym = TRUE) # Alternative formulation of neighbors, pretty similar to sphere of influence approach below
# plot(nb5.PUD.s.p,coords.df250PUD.s.p)

trinb.PUD.s.p<-tri2nb(coords.df250PUD.s.p) # This approach will create symmetric neighbors
